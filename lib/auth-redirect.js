...
export function setupAuthRedirect() {
    // Verifica se estamos no browser antes de executar o código
    if (typeof window !== 'undefined') {
        ...
        const cleanupAuthRedirect = () => {
            ...
            if (window.location.hash && window.location.hash.includes('access_token')) {
                ...
                // Armazenar um sinalizador para evitar redirecionamentos cíclicos
                if (!sessionStorage.getItem('auth_redirect_processed')) {
                    sessionStorage.setItem('auth_redirect_processed', 'true');

                    // Redirecionar para a página de dashboard se estivermos na raiz ou página de login
                    ...
                    console.log('[Auth] Redirecionamento já processado, evitando loop');
                    sessionStorage.removeItem('auth_redirect_processed');
                }

                // Limpa o hash mas mantém a rota atual
